-- An option could say the same thing twice: "Triceps does more of the work" (computed from the
-- muscle rows) next to "More triceps" (the rule's own sentence). Each sentence now carries the
-- muscle it is about, so the app can drop its generated line wherever a rule already says it.
...
language sql stable set search_path = '' as $$
  ...
  same_exercise as (   -- levels 1 and 2: one row per visible difference
    select distinct on (c.attrs, c.equipment_ids, c.variant_label)
           ...
           round(1.0 / (1 + cardinality(array(select unnest(c.attrs) except select unnest(me.attrs)))
                          ...
                          + 0.1 * (cardinality(array(select unnest(c.equipment_ids) except select unnest(me.equipment_ids)))
                                   + cardinality(array(select unnest(me.equipment_ids) except select unnest(c.equipment_ids))))
                          + 0.005 * c.equipment_rank), 3) as score,
           c.variant_label
  ...
  select r.level, r.variant_id, r.exercise_id, v.names, r.equipment_ids, r.basis, r.score,
         ...
         coalesce((select jsonb_agg(e.notes || jsonb_build_object('muscle_id', e.muscle_id)
                            order by e.sort_order) from public.variant_emphasis e
