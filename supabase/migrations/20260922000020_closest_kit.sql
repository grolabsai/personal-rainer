-- Level 1 and 2 were ranked by how many pieces of kit an option needs, which put a lever machine
-- above "Dumbbell + Bench" as a swap for a barbell bench press. The closest swap is the one that
-- keeps most of the setup you already have out, so the ranking is now the difference between the
-- two kits (symmetric difference), not the size of the new one.
